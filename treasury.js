...
function renderPurchaseHistory(purchases) {
  ...
  list.innerHTML = purchases.map((p, i) => {
    ...
    const xLinkHTML = p.tokenX
      ? `<a href="${p.tokenX}" target="_blank" rel="noopener" class="purchase-x-link">View ${p.tokenSymbol} on X ↗</a>`
      : '';
    ...
    return `
      <div class="purchase-card">
        ...
        ${xLinkHTML}
        <button class="purchase-share-btn" data-index="${i}">Share this ↗</button>
      </div>`;
  }).join('');
